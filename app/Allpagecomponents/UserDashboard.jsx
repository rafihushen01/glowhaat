
"use client"
import React from 'react'
import UserNav from './UserNav'
import UserHomebanner from './UserHomebanner'
import NewItem from "../Allpagecomponents/NewItem"
import HomepageManagedSections from './HomepageManagedSections'
import BehaviorRecommendations from './BehaviorRecommendations'
import BrandFooter from './BrandFooter'


const UserDashboard = () => {
  return (
    <div className="min-h-screen bg-white">
      <UserNav />
      <main className="pt-[140px] lg:pt-[196px]">
        <section className="mx-auto mt-24 w-full max-w-[1920px] px-2 sm:px-4 lg:px-6">
          <UserHomebanner />
        </section>
        <section className="mx-auto mt-5 w-full max-w-[1920px] px-2 sm:px-4 lg:px-6">
          <HomepageManagedSections />
        </section>
        <section id="new-arrivals">
          <NewItem/>
        </section>
        <section id="deals-you-cant-miss">
          <BehaviorRecommendations />
        </section>
      </main>
      <BrandFooter />
    </div>
  )
}

export default UserDashboard
