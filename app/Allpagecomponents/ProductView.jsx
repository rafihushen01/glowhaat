...
import { 
  ...
  Heart, Maximize2, X, PlayCircle, ZoomIn, ZoomOut, RotateCcw 
} from 'lucide-react'
...
const ProductView = () => {
 ...
 const handleScroll = () => {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center text-purple-900 animate-pulse">Loading Rafis World...</div>;
  ...
  return (
    <div className="min-h-screen bg-white font-sans text-gray-900 selection:bg-purple-100">
      ...
      <nav className="w-full max-w-7xl mx-auto px-4 py-4 text-sm text-gray-500 flex items-center gap-2">
        <span className="cursor-pointer hover:text-purple-700" onClick={() => router.push('/')}>Home</span>
        <ChevronRight size={14} />
        <span className="cursor-pointer hover:text-purple-700" onClick={() => router.push('/products')}>Products</span>
        <ChevronRight size={14} />
            ...
            <div className="flex md:flex-col gap-3 overflow-x-auto md:overflow-y-auto md:w-24 scrollbar-hide">
              {currentVariant?.images.map((img, idx) => (
                <button 
                  ...
                  className={`relative w-20 h-24 md:w-full md:h-28 flex-shrink-0 border transition-all duration-300 ${mainImage === img ? 'border-purple-800 ring-1 ring-purple-800' : 'border-gray-200 opacity-70 hover:opacity-100'}`}
                >
              ...
              {isVideoAvailable && (
                 <button className="relative w-20 h-24 md:w-full md:h-28 flex-shrink-0 border border-gray-200 bg-gray-50 flex items-center justify-center group">
                   <PlayCircle className="text-gray-400 group-hover:text-purple-600" />
                 </button>
              )}
            </div>
...
<div className="absolute bottom-3 left-1/2 -translate-x-1/2 bg-white/75 backdrop-blur-[2px] px-3 py-1.5 rounded-full shadow-sm opacity-0 group-hover:opacity-100 transition-all duration-500 ease-out flex items-center gap-1.5 pointer-events-none transform translate-y-4 group-hover:translate-y-0">
  <Maximize2 size={12} className="text-purple-900" />
  <span className="text-[9px] font-bold uppercase tracking-widest text-purple-900 leading-none mt-[1px]">
    Tap to Zoom
  </span>
</div>
              ...
              <div className="absolute top-4 left-4 flex flex-col gap-2">
                 ...
                 {product.flashsale && <span className="bg-purple-600 text-white text-xs font-bold px-3 py-1 uppercase ">Flash Sale</span>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col pt-2">
          <div className="mb-6 border-b border-gray-100 pb-6">
            <h2 className="text-purple-800 text-sm font-bold tracking-widest uppercase mb-2">Rafis World Exclusive</h2>
             ...
             <div className="flex flex-wrap gap-3">
               {product.variants.map((variant, idx) => (
                 <button
                   ...
                   className={`w-12 h-12 rounded-full border-2 flex items-center justify-center transition-all ${selectedVariantIndex === idx ? 'border-purple-800 p-1' : 'border-transparent'}`}
                 >
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center mb-3">
              ...
              <button className="text-xs text-purple-700 underline">Size Guide</button>
            </div>
            <div className="grid grid-cols-4 md:grid-cols-6 gap-3">
              {currentVariant?.options.map((opt, idx) => (
                <button
                  ...
                  className={`py-3 text-sm font-medium transition-colors border ${selectedOptionIndex === idx ? 'bg-purple-900 text-white border-purple-900' : 'bg-white text-gray-900 border-gray-200 hover:border-purple-900'}`}
                >
                  {opt.name}
                </button>
              ))}
      ...
      <div className="relative group">
        ...
        <div className="flex items-center border border-gray-200 h-[60px] bg-white overflow-hidden shadow-sm hover:border-purple-800 transition-colors rounded-sm">
          ...
          <div className="w-24 h-full relative bg-gray-50 border-r border-gray-100">
              ...
              {Array.from({ length: 140 }, (_, i) => i + 1).map((num) => (
                <div 
                  ...
                  className={`h-[60px] flex items-center justify-center snap-start shrink-0 transition-all duration-200 ${
                    quantity === num 
                      ? "text-2xl font-bold text-purple-900 scale-110" 
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-12 h-8 border-y border-purple-100 rounded pointer-events-none opacity-50"></div>
          </div>
          ...
          <div className="flex flex-col h-full w-10 bg-white">
            <button 
              ...
              className="flex-1 flex items-center justify-center hover:bg-purple-50 text-gray-400 hover:text-purple-900 transition-colors border-b border-gray-100 active:bg-purple-100"
            >
              <Plus size={14} />
            </button>
            <button 
              onClick={handleDecrement}
              className="flex-1 flex items-center justify-center hover:bg-purple-50 text-gray-400 hover:text-purple-900 transition-colors active:bg-purple-100"
            >
              <Minus size={14} />
            </button>
          </div>
      ...
      <button className="flex-1 w-full bg-purple-900 hover:bg-purple-800 text-white text-sm uppercase tracking-[0.2em] font-bold h-[60px] flex items-center justify-center gap-3 group/btn relative overflow-hidden shadow-xl shadow-purple-900/10">
              ...
              {({ zoomIn, zoomOut, resetTransform }) => (
                <React.Fragment>
                  ...
                  <div className="absolute bottom-10 left-1/2 -translate-x-1/2 z-50 flex gap-4 bg-white/90 backdrop-blur px-6 py-3 rounded-full shadow-2xl border border-gray-200">
                    ...
                    <button onClick={() => resetTransform()} className="p-2 hover:bg-gray-100 rounded-full text-purple-700 font-bold text-xs uppercase" title="Reset">
                       Reset
                    </button>
                    <div className="w-px bg-gray-300 h-6 my-auto"></div>
